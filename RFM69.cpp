...
#ifdef RFM69_OOK
bool RFM69::initialize_OOK(byte freqBand)
{
    pinMode(_interruptPin, INPUT);
#ifdef RFM69_OOKLONE
    pinMode(_interruptPin, INPUT);
    pinMode(RF69_IRQ_DATA_PIN, INPUT);
    pinMode(RF69_DIO1_DATA_PIN, INPUT);
    const byte CONFIG[][2] =
    {
		...
		// #define RFM69_BITRATE_KBPS 32.0; rfm69_write_bitrate((uint16_t)(32000.0 / RFM69_BITRATE_KBPS));
		/* 0x03 */ { REG_BITRATEMSB, RF_BITRATEMSB_32768}, //maximum ook bitrate is 32 Kbps
		/* 0x04 */ { REG_BITRATELSB, RF_BITRATELSB_32768}, //maximum ook bitrate is 32 Kbps
		// 433.92 MHz carrier rfm69_write_frf((uint32_t)7109345.28)
		...
		///* 0x11 */ { REG_PALEVEL, RF_PALEVEL_PA0_ON | RF_PALEVEL_PA1_OFF | RF_PALEVEL_PA2_OFF | RF_PALEVEL_OUTPUTPOWER_11111},
		//   rfm69_write_lna(x) x = (1 << 7) | (1 << 0);
		/* 0x18 */ { REG_LNA, RF_LNA_ZIN_200 | RF_LNA_GAINSELECT_AUTO}, //LNA input impedance 200 Ohm, auto gain select
		// bits<7:5>: dcc freq bits<4:3>: rx bw mant bits<2:0>: rx bw exp rfm69_write_rx_bw((2 << 5) | (1 << 3) | (1 << 0));
    ...
    const byte CONFIG[][2] =
    {
      ...
      ///* 0x13 */ { REG_OCP, RF_OCP_ON | RF_OCP_TRIM_95 }, //over current protection (default is 95mA)
       ...
       //   /* 0x19 */ { REG_RXBW, RF_RXBW_DCCFREQ_010 | RF_RXBW_MANT_24 | RF_RXBW_EXP_5 }, //
        ...
        /* 0x1B */ { REG_OOKPEAK, RF_OOKPEAK_THRESHTYPE_PEAK | RF_OOKPEAK_PEAKTHRESHSTEP_000 | RF_OOKPEAK_PEAKTHRESHDEC_000 }, //74
        /* 0x1C */ { REG_OOKAVG, 80 },//80   //Geen idee waarvoor
...
//  SPI.setDataMode(SPI_MODE0);
  ...
  // Bovenstaande configuratie schrijven naar RFM69
...
#ifdef RFM69_OOK
/// Turns the radio on or off in OOK mode
bool RFM69::radioOnOff(bool onoff) {
    if (onoff) {
       // setMode(RF69_MODE_TX);
        //writeReg(REG_OPMODE, (readReg(REG_OPMODE) & 0xE3) | RF_OPMODE_TRANSMITTER_SEQUENCER_OFF);
        writeReg(REG_DATAMODUL, (3 << 5) | (1 << 3));
        writeReg(REG_OPMODE, (1 << 7) | (4 << 2));
    } else {
       // setMode(RF69_MODE_STANDBY);
        writeReg(REG_OPMODE, (readReg(REG_OPMODE) & 0xE3) | RF_OPMODE_STANDBY_SEQUENCER_OFF);
    }
